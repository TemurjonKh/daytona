...
import type { DateCandidate } from "@/lib/dates/evidence";
import OpenAI from "openai";
import { zodResponseFormat } from "openai/helpers/zod";
import { ExtractionDraft,extractionSchema,dateEvidenceInstructions } from "./extraction-draft";
...
export async function extractText(source:Source,goal?:string):Promise<OpportunityResult & {importantDates:DateCandidate[]}> {
  ...
  for(let attempt=0;attempt<3;attempt++){
    try {
      ...
      const schema=extractionSchema(simplified);
      const response=await client.chat.completions.parse({model,messages:[{role:"developer",content:instructions+"\n"+dateEvidenceInstructions},{role:"user",content:JSON.stringify({sourceUrl:source.url,pageTitle:source.title,currentDate:new Date().toISOString().slice(0,10),userGoal:goal?.slice(0,1000)||null,sourceTextStart:'BEGIN UNTRUSTED SOURCE DATA',sourceText:source.text.slice(0,15000),sourceTextEnd:'END UNTRUSTED SOURCE DATA'})}],response_format:zodResponseFormat(schema,"opportunity_result")},{signal,timeout:45000});
      const parsed=response.choices[0]?.message.parsed;
      if(!parsed)throw new Error('OpenAI did not return a structured result');
      return ExtractionDraft.parse(parsed);
    }catch(error){
