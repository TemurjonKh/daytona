...
export async function investigateImage(bytes:Buffer,mime:string,emit:Emit,goal?:string){
  ...
  try{
    ...
    const format=()=>zodResponseFormat(extractionSchema(simplified),'opportunity_result');
    const call=()=>client.chat.completions.parse({model,messages:[{role:'developer',content:instructions+'\n'+dateEvidenceInstructions},{role:'user',content:[{type:'text',text:JSON.stringify({sourceIdentifier:id,goal:goal?.slice(0,1000)||null})},{type:'image_url',image_url:{url:`data:${mime};base64,${bytes.toString('base64')}`,detail:'high'}}]}],response_format:format()},{timeout:45000,signal});
    let response;
    ...
    const extracted=ExtractionDraft.parse(response.choices[0]?.message.parsed);
    ...
    const {result:draft,dropped}=normalizeImageResult(extracted,id);
