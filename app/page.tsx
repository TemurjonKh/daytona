...
import { migrateSavedEvents, upsertSavedEvent } from "@/lib/saved-events";
...
export default function Home() {
  ...
  useEffect(() => {
    try {
      const stored=localStorage.getItem("deadline-events");
      const migrated=migrateSavedEvents(stored?JSON.parse(stored):[],detectedTimezone());
      setEvents(migrated);localStorage.setItem("deadline-events",JSON.stringify(migrated));
    } catch {setEvents([]);setNotice("Saved opportunities could not be read. The stored data was not overwritten.");}
  },[]);
  function saveEvent(event:SavedEvent) {
    setEvents(previous=>{const next=upsertSavedEvent(previous,event);try{localStorage.setItem("deadline-events",JSON.stringify(next));}catch{setNotice("Event saved on the server, but browser storage is unavailable.");}return next;});
  }
