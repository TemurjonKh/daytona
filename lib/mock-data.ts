...
export function mockSavedEvents(now: number): SavedEvent[] {
  return [{title: "KRAFTON AI Internship", organization: "KRAFTON", hours: 52}, {title: "Samsung Scholarship", organization: "Samsung", hours: 155}].map((item, i) => SavedEvent.parse({id: `demo-${i}`, title: item.title, organization: item.organization, dueAt: new Date(now + item.hours * 3600000).toISOString(), kind: "deadline", reminderAt: new Date(now + (item.hours - 24) * 3600000).toISOString(), sourceUrl: source, confidence: "high"}));
}
