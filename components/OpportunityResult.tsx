...
export default function OpportunityResult({result,onSave,onTryAgain,live=false}: {live?:boolean;result: Result; onSave: (event: SavedEvent) => void; onTryAgain: () => void}) {
  ...
  const posterEvent = result.sources.some(s=>s.url.startsWith("urn:poster:")) ? result.importantDates.find(d=>d.kind==="event_start"&&d.value) : undefined;
  ...
  async function save() {
    ...
    const event: SavedEvent = {id:crypto.randomUUID(),title:result.title,organization:result.organization,dueAt:rolling ? null : date?.value ?? posterEvent?.value ?? null,kind:rolling ? "rolling" : date ? "deadline" : posterEvent?.kind ?? "deadline",reminderAt:new Date(when).toISOString(),sourceUrl:date?.sourceUrl ?? result.sources[0].url,confidence:result.confidence};
    setBusy(true);
    ...
    try {
      ...
      if (allowed !== "granted") {setMessage(allowed === "unsupported" ? "Notifications unsupported. Event saved." : `Notification permission: ${allowed}. Event saved; allow notifications and save again to schedule.`);return;}
